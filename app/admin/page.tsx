...
<div className="card">
  ...
  <div className="mb-4 text-xl font-bold text-green-600">
    Online Users: {
      [...new Set(devices.filter(d => d.is_online).map(d => d.member_name))].length
    }
  </div>

  <div className="mb-2 text-md text-gray-700">
    {
      [...new Set(devices.filter(d => d.is_online).map(d => d.member_name))]
        .join(", ") || "No users online"
    }
  </div>
  ...
  <div className="mb-4 text-lg font-bold text-blue-600">
    Online Families: {
      [...new Set(devices.filter(d => d.is_online).map(d => d.family_code))].length
    }
  </div>

  <div className="mb-2 text-md text-gray-700">
    {
      [...new Set(devices.filter(d => d.is_online).map(d => d.family_code))]
        .join(", ") || "No families online"
    }
  </div>
